...
public class Maze {
    ...
    static int loopForGameRounds = 0;
    public static void main(String[] args) {
        makeBoard();//makes the physical board

        displayBoard();
        while(loopForGameRounds==0){
            userInput();
            displayBoard();
            checkForEnd();
    ...
    public static void checkForEnd() {
        ...
        if (winLoseOrDraw == 1){
            ...
            loopForGameRounds++;//breaks loop in main = game stops
        }
    ...
    private static void checkThreeInARow() {
        for (int row = 0; row < 3; row++){//iterates through each row
            if (checkThreeSame(board[row][0], board[row][1], board[row][2], "X") == true){//if there's 3 X's in a row
                winLoseOrDraw++;
            }
            else if (checkThreeSame(board[row][0], board[row][1], board[row][2], "O") == true){//if there's 3 O's in a row
                winLoseOrDraw = 2;
            }
        }
    ...
    private static void checkThreeInAColumn() {
        for (int column = 0; column < 3; column++){//iterates through each column
            if (checkThreeSame(board[0][column], board[1][column], board[2][column], "X") == true){//if there's 3 X's in a column
                winLoseOrDraw++; 
            }
            else if (checkThreeSame(board[0][column], board[1][column], board[2][column], "O") == true){//if there's 3 O's in a column
                winLoseOrDraw = 2; 
            }
        }
    ...
    private static void checkThreeInADiagonal() {
        if (checkThreeSame(board[0][0], board[1][1], board[2][2], "X") == true || checkThreeSame(board[0][2], board[1][1], board[2][0], "X") == true){//if there's 3 X's in a diagonal
            winLoseOrDraw ++;
        }
        else if (checkThreeSame(board[0][0], board[1][1], board[2][2], "O") == true || checkThreeSame(board[0][2], board[1][1], board[2][0], "O") == true){//if there's 3 O's in a diagonal
            winLoseOrDraw = 2;
        }
    }
...
//Computer-related
    ...
    public static void computerAction(){
        ...
        if (checkTwoInARowComputer("O") == true){//parameter "O" tells it to identify if there are two O's in a row (see method for specifics)
        ...
        else if (checkTwoInARowComputer("X") == true){//parameter "X" tells it to identify if there are two X's in a row
    ...
    private static boolean checkTwoInARowComputer(String XorO) {
        for (int row = 0; row < 3; row++){
            if (checkTwoSame(board[row][0], board[row][1], XorO) == true ||//if left and center in same row are same string
                checkTwoSame(board[row][1], board[row][2], XorO) == true ||//if center and right are same string
                checkTwoSame(board[row][0], board[row][2], XorO) == true){//if left and right are same string
    ...
    private static boolean checkTwoInAColumnComputer(String XorO) {
        for (int column = 0; column < 3; column++){
            if (checkTwoSame(board[0][column], board[1][column], XorO) == true ||//goes through each possiblity of there being two of the same in a column
    ...
    private static boolean checkTwoInADiagonalComputer(String XorO) {
        ...
        if (checkTwoSame(board[0][0], board[1][1], XorO) == true ||//goes through each possibility of two of the same in left diagonal
        ...
        else if (checkTwoSame(board[0][2], board[1][1], XorO) == true ||//goes through each possibility of two of the same in right diagonal
    ...
    //last priority: plays random hole
